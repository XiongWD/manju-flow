...
(async() => {
  ...
  try {
    ...
    log('projects edit action exists', await count(page.getByRole('button', { name: /编辑/ })) > 0, `count=${await count(page.getByRole('button', { name: /编辑/ }))}`);
    log('projects delete action exists', await count(page.getByRole('button', { name: /删除/ })) > 0, `count=${await count(page.getByRole('button', { name: /删除/ }))}`);

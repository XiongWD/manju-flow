              ...
              <div className="pb-4">
                <GlassSurface variant="elevated" padded>
                  <div className="space-y-3">
                    ...
                    {asset.mime_type ? <p className="truncate text-xs text-zinc-500">{asset.mime_type}</p> : null}
                    <div className="flex items-center gap-2">
                      {asset.metadata_json?.object_name ? (
                        <a href={getAssetServeUrl(asset.id)} target="_blank" rel="noopener noreferrer" className="inline-block text-xs text-violet-400 hover:underline">打开资源</a>
                      ) : isMockUri(asset.uri) ? (
                        <span className="text-xs text-zinc-600">模拟数据，无文件</span>
                      ) : asset.uri ? (
                        <a href={asset.uri} target="_blank" rel="noopener noreferrer" className="inline-block text-xs text-violet-400 hover:underline">打开资源</a>
                      ) : (
                        <span className="text-xs text-zinc-600">无可访问链接</span>
                      )}
                      <button onClick={() => onPreview(asset)} className="rounded-md bg-zinc-700 px-3 py-1 text-xs font-medium text-zinc-200 hover:bg-zinc-600">
                        预览
                      </button>
                    </div>
                    <p className="text-xs text-zinc-600">{new Date(asset.created_at).toLocaleString("zh-CN")}</p>
                  </div>
                </GlassSurface>
              </div>
            )}
